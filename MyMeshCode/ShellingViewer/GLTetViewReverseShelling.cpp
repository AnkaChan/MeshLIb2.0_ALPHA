...
#define SOLID_MODE 1
#define WIRE_MODE 2
#define SHELLING_MODE 1
#define REVERSE_SHELLING_MODE 2
...
/*! draw the remaining half faces in shelling procedure*/
void draw_half_faces_reverseShelling()
{
	std::list<CHalfFaceGL *> &HalfFaces = pMesh->half_faces();
	TetCenter = shellingList->front()->vertex(0)->position();

	//glBindTexture(GL_TEXTURE_2D, texName);
	glBegin(GL_TRIANGLES);

	for (auto renderTetIter = HalfFaces.begin(); renderTetIter != HalfFaces.end(); ++renderTetIter)
	{
		CHalfFaceGL * pHF = *renderTetIter;
		CHalfFaceGL *pDualHF = pMesh->HalfFaceDual(pHF);

		CTetGL *pT = pMesh->HalfFaceTet(pHF);
		if (pT->visible) {
			continue;
		}
		if (pDualHF != NULL) {
			CTetGL *pDualT = pMesh->HalfFaceTet(pDualHF);
			if (!pDualT->visible) {
				continue;
			}
		}
...
void display()
{
	...
	setupObject();
	if (renderMode == SHELLING_MODE)
		draw_half_faces_shelling();
	else
		draw_half_faces_reverseShelling();
	draw_axis();
	if (drawCircumSphere)
...
void keyBoard(unsigned char key, int x, int y)
{
	...
	case 'b':
		if (renderMode == SHELLING_MODE)
			renderMode = REVERSE_SHELLING_MODE;
		else
			renderMode = SHELLING_MODE;
		break;
	case '?':
		help();
		break;
