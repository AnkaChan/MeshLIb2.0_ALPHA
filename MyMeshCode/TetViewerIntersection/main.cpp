...
#include "../IntersectionDetectionDemo/IntersectionDetection.h"
#include <iostream>
...
int main(int argc, char **argv){
	...
	for (auto pT : TIT::TM_TIterator(pTMesh)) {
		...
		if (orientedVolume(pT) < 0) {
			std::cout << "Negative oriented volume." << std::endl;
	...
	int numIntersectionDetected = 0;
	CIntersectionDetection IntersectionDetector;
	for (auto pE : TIT::TM_EIterator(pTMesh)) {
		bool pEOnPositiveT = true;
		for (auto pTE : TIT::E_TEIterator(pE)) {
			TIF::TPtr pT = TIF::TEdgeTet(pTE);
			if (orientedVolume(pT) < 0) {
				pEOnPositiveT = false;
			}
		}
		...
		if (pEOnPositiveT) {
			for (auto pHF : TIT::TM_HFIterator(pTMesh)) {
				...
				bool skip = false;
				TIF::TPtr pT = TIF::HalfFaceTet(pHF);
				for (auto pEOnT : TIT::T_EIterator(pT)) {
					if (pEOnT == pE) {
						skip = true;
					}
				}
				...
				CPoint l1 = TIF::EdgeVertex1(pE)->position();
				CPoint l2 = TIF::EdgeVertex2(pE)->position();

				CPoint v[3];
				TIF::HalfFace3Points(pHF, v);

				IntersectionDetector.setLineSegment(l1, l2);
				IntersectionDetector.setTriangle(v[0], v[1], v[2]);

				if (IntersectionDetector.detectApproximately()) {
					if (orientedVolume(pT) > 0) {
						hasIntersection = true;
						++numIntersectionDetected;
						cout << "Intersection detected: " << numIntersectionDetected << " times.\n";
					}
				}
