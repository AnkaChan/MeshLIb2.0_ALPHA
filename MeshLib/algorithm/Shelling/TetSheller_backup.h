...
namespace MeshLib{
	namespace TMeshLib{
		...
		public:
			...
			void biShellingBreadthFirstGreedy(std::list<T*> & sourceList, int getFirstNOnly = 0);
		...
		template<typename TV, typename V, typename HE, typename TE, typename E, typename HF, typename F, typename T>
		void CTetSheller<TV, V, HE, TE, E, HF, F, T>::biShellingBreadthFirstGreedy(std::list<T*>& sourceList, int getFirstNOnly)
		{
			...
			while (!candidateList.empty()) {
				...
				if (getFirstNOnly != 0 && m_pShellingOrder->size() > getFirstNOnly) {
					cout << "The first " << getFirstNOnly << " simplices in shelling procedure has been found." << endl;
					break;
				}
			}
			shellingDone = true;
		...
		bool CTetSheller<TV, V, HE, TE, E, HF, F, T>::isShelling(T * newSimplex)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				for (TVertexInHalfEdgeIterator TvHfItr(pMesh, pTV); !TvHfItr.end(); ++TvHfItr) {
					HE* pHe = *TvHfItr;
				...
				{
					V* pV = pTV->vert();
					for (VertexTVertexIterator VtVItr(pMesh, pV); !VtVItr.end(); ++VtVItr)
					{
						TV* pTV = *VtVItr;
						T* pTet = (T*)pTV->tet();
			...
			for (TetEdgeIterator TEItr(pMesh, newSimplex); !TEItr.end(); ++TEItr) {
				possibleSingular = true;
				E *pE = *TEItr;
				TE *pTE = NULL;
		...
		inline bool CTetSheller<TV, V, HE, TE, E, HF, F, T>::edgeInFace(E * pE, F * pF)
		{
			...
			for (HalfFaceHalfEdgeIterator HFHEITer(pMesh, pHF); !HFHEITer.end(); ++HFHEITer) {
				HE *pHE = *HFHEITer;
		...
		inline bool CTetSheller<TV, V, HE, TE, E, HF, F, T>::isBoundaryEdgeInShelling(E * pE)
		{
			if (pE->boundary())
				return true;
			for (EdgeTEdgeIterator ETIter(pMesh, pE); !ETIter.end(); ++ETIter) {
				TE *pTE = *ETIter;
